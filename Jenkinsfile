pipeline {
  ...
  environment {
    ...
    awsecrRegistry = '637423550581.dkr.ecr.us-west-2.amazonaws.com/eks-demo-repo'
    awsecrRegistryCredential = 'credential-AWS-ECR'
    githubCredential = 'credential-github'
    gitEmail = 'finalcloudproject4th@gmail.com'
  ...
  stages {
    ...
    stage('Checkout Application Git Branch') {
      steps {
        checkout([$class: 'GitSCM', branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: githubCredential, url: 'https://github.com/finalcloudproject4th/kustomize.git']]])
      }
      // steps 가 끝날 경우 실행한다.
    ...
    stage('Docker Image Push') {
      steps {
        ...
        withDockerRegistry([url: "https://${awsecrRegistry}", credentialsId: "ecr:us-west-2:${awsecrRegistryCredential}"]) {
    ...
    stage('Deploy') { 
      steps {
        ...
        git credentialsId: githubCredential,
            url: 'https://github.com/finalcloudproject4th/kustomize.git',
            branch: 'main'  
    ...
    stage('Push to Git Repository') {
      steps {
        withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: githubCredential, usernameVariable: 'GIT_USERNAME', passwordVariable: 'GIT_PASSWORD']]) {
          sh "git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/finalcloudproject4th/kustomize.git"       
        }
      }
      post {
                failure {
